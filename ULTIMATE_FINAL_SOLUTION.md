...
- **الحل:** 
  - استخدام `--set-cloudsql-instances` بدلاً من `--add-cloudsql-instances`
  - استخدام نفس الصيغة من `run-migrations-cloud-run-job-with-password.ps1` التي تعمل
  - استخدام Invoke-Expression مع string building
...
### 1. استخدام --set-cloudsql-instances
```powershell
# قبل (خطأ):
...
### 2. NODE_ENV Cleanup
```powershell
# Always set NODE_ENV to "production" only
$envVarsList += "NODE_ENV=production"
# Skip NODE_ENV from service (may be corrupted)
```

### 3. Environment Variables Filtering
```powershell
# Skip PORT, FRONTEND_URL, BACKEND_URL
# Keep only essential variables for migrations
```
