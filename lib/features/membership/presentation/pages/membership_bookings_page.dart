...
class MembershipBookingsPage extends StatelessWidget {
  const MembershipBookingsPage({
    super.key,
  ...
  Widget build(BuildContext context) {
    ...
    final subscriptions =
        (bookingsData['subscriptions'] as List<dynamic>? ?? [])
            .cast<Map<String, dynamic>>();
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(
          showOnlyCompleted
              ? 'الحجوزات المكتملة - $membershipName'
              : 'الحجوزات المتعلقة - $membershipName',
        ),
      ),
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
                    ...
                    children: [
                      ...
                      Expanded(
                        child: _SummaryItem(
                          label: 'الاشتراكات',
                          value:
                              (summary['totalSubscriptions'] ?? 0).toString(),
                          color: Colors.blue,
                        ),
                      ),
                    ],
            ...
            if (subscriptions.isNotEmpty && !showOnlyCompleted) ...[
              Text(
                'الاشتراكات',
                style: Theme.of(context).textTheme.titleLarge?.copyWith(
                      fontWeight: FontWeight.bold,
              ...
              const SizedBox(height: 16),
              ...subscriptions.map((subscription) => _SubscriptionCard(
            ...
            children: [
              Expanded(
                child: _InfoItem(
                  ...
                  label: 'المبلغ',
                  value:
                      '${parseDouble(booking['totalPrice']).toStringAsFixed(0)} EGP',
                ),
              ),
              if (date != null)
                Expanded(
...
class _SubscriptionCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final statusColor = _getStatusColor(status);
    final date = subscription['createdAt'] != null
        ? DateTime.tryParse(subscription['createdAt'].toString())
        : null;
            ...
            children: [
              Expanded(
                child: Column(
                  ...
                  children: [
                    Text(
                      subscription['userName']?.toString() ??
                          'مستخدم غير معروف',
                      style: Theme.of(context).textTheme.titleMedium?.copyWith(
                            fontWeight: FontWeight.bold,
                          ),
                    ...
                    if (subscription['userEmail'] != null)
                      Text(
                        subscription['userEmail'].toString(),
                        style: Theme.of(context).textTheme.bodySmall?.copyWith(
                              color: Colors.grey[600],
                            ),
                      ),
            ...
            children: [
              Expanded(
                child: _InfoItem(
                  ...
                  label: 'المبلغ',
                  value:
                      '${parseDouble(subscription['amount']).toStringAsFixed(0)} EGP',
                ),
              ),
              if (date != null)
                Expanded(
